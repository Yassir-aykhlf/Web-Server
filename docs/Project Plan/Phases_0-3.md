...
*   **The Goal:** Create a main `Config` class.
    *   It essentially holds a `std::vector<ServerConfig>`.
...
*   **The Goal:** Implement the logic that iterates through your tokens and fills the classes created in B1, B2, and B3.
    ...
    *   It handles the nesting (Location is *inside* Server).
...
*   **The Goal:** Sanity check the data *after* parsing but *before* the server starts.
...
*   **The Purpose:** The subject requires that the server works even if the user provides *no* configuration for error pages. If a 404 happens, you must show something.
...
*   **The Purpose:** `poll` only gives you a File Descriptor ID (an integer). You need an object to store the *context* associated with that integer.
*   **The Goal:** Create the "Session Object."
    ...
    *   *Why Buffers?* You might read a request halfway. You need a place to store "Part 1" until "Part 2" arrives.
...
*   **The Goal:** Create a "Data Object" (a class with mostly getters and setters).
    ...
    *   It does **not** contain logic. It just holds the data so Student 1 (Router) and Student 2 (Server) can ask questions like: *"Does this request have a body?"* or *"What is the Host header?"*
...
*   **The Goal:** Parse the key-value pairs.
    ...
    *   **Validation:** If it is HTTP/1.1, you *must* ensure the `Host` header exists.
...
*   **The Goal:** Read the correct amount of data.
    ...
    *   If you receive more, leave the extra bytes for the *next* request (pipelining).
...
*   **`Config.cpp`** (The Container)
    *   **Member:** `std::vector<ServerConfig> _servers`.
...
*   **`HttpParser.cpp`** (The Logic)
    ...
    *   **Method:** `parse(std::string &raw_data, HttpRequest &req)`:
...
*   **`HttpResponse.cpp`** (The Builder)
    ...
    *   **Method:** `build()`: Concatenates everything into a `std::string` ready for Student 2 to send.
